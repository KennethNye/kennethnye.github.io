...
Then, for each $\omega$, we get a counting measure because it satisfies equation (2). For each $A \in \mathcal{B}$, we get a random variable because summation and composition with ${\mathrm{1}_{A}}$ are measurable. The point process is simple as long as the ${X_{n}}$ s are distinct with probability 1.
