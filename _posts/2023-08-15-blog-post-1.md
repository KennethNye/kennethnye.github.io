...
tags:
  - research
---
# Multiple Hypothesis Tracking in Summary
...
$\mathbf{x}^{k} = \left\{ \mathbf{x}(t) \right\}_{t = 1,\cdots,k}$. In
particular $\mathbf{x}_{j}^{k}$ is the sequence of (estimated) states
associated with the $j$-th track.
...
$\mathbf{z}^{k} = \left\{ \mathbf{z}(t) \right\}_{t = 1,\cdots,k}$. In
particular, $\mathbf{z}_{j}^{k}$ is the sequence of measurements for the
$j$-th track. An example of a track up to frame $k - 1$ is shown as
$\mathbf{z}_{1}^{k - 1} = \{\cdots,\mathbf{z}_{2}(k - 1),\mathbf{z}_{3}(k - 4),\mathbf{z}_{1}(k - 3),\mathbf{z}_{3}(k - 2),\mathbf{z}_{1}(k - 1)\}$
in Fig. 1.

In MTT, a tracking hypothesis up to time $k$ is defined as a set of $m$
tracks $\Theta^{k} = \left\{ \theta_{j}^{k} \right\}_{j = 1,\cdots,m}$,
each one consisting of some sequence of measurements and estimated
states:
$\theta_{j}^{k} = \left\{ \mathbf{z}_{j}^{k},\mathbf{x}_{j}^{k} \right\}$.
...
1, there are 5 possible extension tracks $\mathbf{z}_{1}^{k + 1}$s of
the track
$\mathbf{z}_{1}^{k - 1} = \{\cdots,\mathbf{z}_{2}(k - 1),\mathbf{z}_{3}(k - 4),\mathbf{z}_{1}(k - 3),\mathbf{z}_{3}(k - 2),\mathbf{z}_{1}(k - 1)\}$
...
measurements
$Z^{K} = \left\{ \left\{ \mathbf{z}_{j}(k) \right\}_{j = 1,\cdots,n_{k}} \right\}_{k = 1,\cdots,K}$where
...
frame $k$, i.e.
$Z_{0}(k) = \left\{ \mathbf{x} \in \left\{ \mathbf{x}_{i}(k) \right\}_{i = 1,\cdots,n_{k}}:\mathbf{x} \notin \bigcup_{j = 1,\cdots,K}^{}\mathbf{x}_{j}^{K} \right\}$.
...
$p\left( s^{i}(k) \right) = \sum_{j \in \left\{ 0,1 \right\}}^{}{\pi_{ji}p\left( s^{j}(k - 1) \right)}$.
...
P_{FD}(\mathbf{z}(k),k),\ if\ \mathbf{z}(k)\ is\ real \\
1,\ otherwise \\
\end{array} \right.\ $$
...
Denote
$\Gamma_{j}^{k} = \left\{ \theta_{j,\mathbf{z}_{i}}^{k} \right\}_{i = 1,\cdots,n_{k}}$
...
$\Gamma^{k:k + d} = \left\{ \Gamma_{j}^{t} \right\}_{t = k,\cdots,k + d}$.
...
$$\mathcal{B =}\left\{ \mathbf{b}(j) = 1\ or\ 0\ for\ j = 1,\cdots,N \right\}$$
