...
$$= \sum_{i = 1}^{n}{{E_{p\left( z \middle| x,\theta^{(t)} \right)}(z}_{i})\left( \log\pi + x_{i}\log p + \left( 1 - x_{i} \right)\log(1 - p) \right) + \left( 1 - {E_{p\left( z \middle| x,\theta^{(t)} \right)}(z}_{i}) \right)\left( \log(1 - \pi) + x_{i}\log q + \left( 1 - x_{i} \right)\log(1 - q) \right)}$$
